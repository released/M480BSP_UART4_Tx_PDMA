...
#line 3 "..\\misc_config.c"

 

struct flag_8bit flag_MISC_CTL;
#line 15 "..\\misc_config.c"

 

#line 32 "..\\misc_config.c"
...
void compare_buffer(unsigned char *src, unsigned char *des, int nBytes)
{
    ...
    for (i = 0; i < nBytes; i++)
    {
        if (src[i] != des[i])
        {
			...
			(flag_MISC_CTL . bit0) = 1;
        }
    }

	if (!(flag_MISC_CTL . bit0))
	{
    	printf("%s complete \r\n" , __FUNCTION__);	
	}

    printf("%s end \r\n" , __FUNCTION__);
	(flag_MISC_CTL . bit0) = 0;
#line 67 "..\\misc_config.c"
